...
$webTitle = 'Username Search for '. $usernameSearch;

$i = 0;
	

$searchquery = "SELECT distinct(dbIdentifier), db_type FROM `emailsUnique` WHERE `username` = '$usernameSearch' and `db_type` != 'experian_data' and `db_type` != 'experian_newdata' LIMIT 500";
...
$searchquery = "SELECT distinct(dbIdentifier), db_type FROM `emailsUnique` WHERE `username` = '$usernameSearch' and `db_type` != 'main_db' and `db_type` != 'quanki_hostgator' and `db_type` != 'quanki_newdata'  LIMIT 500";
$result  = $db1->query($searchquery);
...
$searchquery = "SELECT uniqueId FROM `email` WHERE `username` = '$usernameSearch' LIMIT 500"; 
...
<head>
	...
	<title><?php echo 'Results For '. ucwords($usernameSearch);?></title>
			...
			<div class="row">
				...
				<div class="dir-alp-tit">
					...
					<ol class="breadcrumb">
						...
						<li class="active" style="color:#fff !important;"><?php echo strtoupper($usernameSearch); ?></li>
    ...
    echo '	  <div class="pglist-p-com-ti">
								...
								<div class="list-pg-write-rev">
									...
									<form name="frm" method="POST" action="" class="col">
										...
										<p>Do you feel like we are missing a username record for <strong>' . strtoupper($usernameSearch) . '? Not a problem, give us your details and we will shoot over an email as soon as we find the result you are looking for:</p>
										...
										<div class="row">
<div class="input-field col s12"><input type="submit" name="Update" id="update" value="Find me details about ' . $usernameSearch .'" class="waves-effect waves-light btn-large full-btn" /></div>
...
?>							
  ...
  var comments = $("#comments").val();
	 var query = '<?php echo $usernameSearch; ?>';
   var type = 'username'; 
