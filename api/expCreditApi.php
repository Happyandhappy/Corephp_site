...
$city = htmlentities(clean($_GET['city']));
$city = str_replace('_',' ',$city);
$state = htmlentities(clean($_GET['state']));
$zip = htmlentities(clean($_GET['zip']));
$credit = htmlentities(clean($_GET['cr']));

//echo $city;

/*if (($city == '' || $city == NULL) || ($state == '' || $state == NULL) || ($zip == '' || $zip == NULL) || ($credit == '' || $credit == NULL)){
header('Location: 404.php');
}*/

$searchquery = "SELECT id, ADDR FROM `datanew` WHERE `CITY` = '$city' AND `ST` = '$state' AND `ZIP` = '$zip' AND `CREDIT_RATING` = '$credit' LIMIT 500"; 
...
$searchquery = "SELECT id, ADDR FROM `data` WHERE `CITY` = '$city' AND `ST` = '$state' AND `ZIP` = '$zip' AND `CREDIT_RATING` = '$credit' LIMIT 500";
