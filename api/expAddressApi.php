...
$street = htmlentities(clean($_GET['str']));
$street = str_replace('-',' ',$street);
$city = htmlentities(clean($_GET['ct']));
$state = htmlentities(clean($_GET['st']));
...
if (($street == '' || $street == NULL) || ($state == '' || $state = NULL) || ($city == '' || $city = NULL)) {
header('Location: 404.php');
}

$searchquery = "SELECT id, ADDR FROM `datanew` WHERE `ADDR` = '$street' AND `CITY` = '$city' AND `ST` = '$state' AND `ZIP` = '$zip' LIMIT 500"; 
...
$searchquery = "SELECT id, ADDR FROM `data` WHERE `ADDR` = '$street' AND `CITY` = '$city' AND `ST` = '$state' AND `ZIP` = '$zip' LIMIT 500";
