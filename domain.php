...
$domainSearch = $_GET['domain'];

if ((($domainSearch == '') || ($domainSearch == NULL))) {
header('Location: 404.php');
} 
$webTitle = 'Domain Search for '. $domainSearch;

$i = 0;
	

$searchquery = "SELECT distinct(dbIdentifier), db_type FROM `emailsUnique` WHERE `domain` = '$domainSearch' and `db_type` != 'experian_data' and `db_type` != 'experian_newdata' LIMIT 500"; 
...
//	foreach ($arr as $value) {
	for($i = 0; $i < $totalcounter ;$i++){
		...
		if ($db_type == 'quanki_hostgator') {
		$dbOldData = DatabaseOldData::getInstanceOldData();
		$db1 = $dbOldData->getConnectionOldData(); 
		$db_InUse = 'hostgator';
		}
		if ($db_type == 'quanki_newdata') {
		$dbDT = DatabaseDT::getInstanceDT();
...
$searchquery = "SELECT db_type, dbIdentifier FROM `emailsUnique` WHERE `domain` = '$domainSearch' and `db_type` != 'main_db' and `db_type` != 'quanki_hostgator' and `db_type` != 'quanki_newdata' LIMIT 500"; 
$result  = $db1->query($searchquery);
...
$totalcounterExperian = count($resultsDBArrayExperian);
  $jCount = $totalcounterExperian;
if ($totalcounterExperian > 0) {
	
//	foreach ($arr as $value) {
	for($i = 0; $i < $totalcounterExperian ;$i++){
		$q = $resultsDBArrayExperian[$i]['profileId'];
		$db_type = $resultsDBArrayExperian[$i]['db_type'];
		if ($db_type == 'experian_data') {
		$dbExperian = DatabaseExperian::getInstanceExperian(); // DiscoverThem
		$dbExperianConnect = $dbExperian->getConnectionExperian();  // Initial DB
		$db_InUse = 'datanew';
		}
		if ($db_type == 'experian_datanew') {
		$dbExperianOld = DatabaseExperianOld::getInstanceExperianOld(); // DiscoverThem
...
$searchquery = "SELECT uniqueId FROM `email` WHERE `domains` = '$domainSearch' LIMIT 500"; 
...
<head>
	...
	<title><?php echo 'Results For '. ucwords($domainSearch);?></title>
			...
			<div class="row">
				...
				<div class="dir-alp-tit">
					...
					<ol class="breadcrumb">
						...
						<li><a href="domain.php?domain=<?php echo $domainSearch;?>"</a> </li>

						<li class="active" style="color:#fff;"><?php echo strtoupper($domainSearch); ?></li>
...
<ul class="collapsible" data-collapsible="accordion" style="margin-top:0px !important;">
							<li>
								...
								<div class="collapsible-body"><ul>
                              	 ...
                              	 <li><a class="active" href="#people">People (<?php if ($profileTotalCombined == 1000) {
    ...
    echo '	  <div class="pglist-p-com-ti">
								...
								<div class="list-pg-write-rev">
									...
									<form name="frm" method="POST" action="" class="col">
										...
										<p>Do you feel like we are missing a domain record for <strong>' . strtoupper($domainSearch) . '? Not a problem, give us your details and we will shoot over an email as soon as we find the result you are looking for:</p>
										...
										<div class="row">
<div class="input-field col s12"><input type="submit" name="Update" id="update" value="Find me details about ' . $domainSearch .'" class="waves-effect waves-light btn-large full-btn" /></div>
...
$uniqueId = '__'.$uniqueId.'&d=i&t='.$db_InUseIntelius.'';
    ...
    echo '            <div class="home-list-pop list-spac">
									...
									<div class="col-md-9 home-list-pop-desc inn-list-pop-desc" style="float:left;"> <a href="profile.php?unique='.$uniqueId.'"><h3>' . $fullname . ', ' . ($dateOfBirth) . '</h3></a>
...
$uniqueId = '__'.$uniqueId.'&d=e&t='.$experianDbType;
    ...
    echo '            <div class="home-list-pop list-spac">
									...
									<div class="col-md-9 home-list-pop-desc inn-list-pop-desc" style="float:left;"> <a href="eprofile.php?unique='.$uniqueId.'"><h3>' . $fullname . ', ' . ($DobFull) . '</h3></a>
...
?>							
  ...
  var comments = $("#comments").val();
	 var query = '<?php echo $domainSearch; ?>';
   var type = 'domain'; 
