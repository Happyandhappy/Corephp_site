(function($) {
    ...
    var personQuery = function() {
        var query = "";
        if (!hasLS && typeof searchData !== "undefined") { query = "?" + $.param(searchData); }
        return query;
    }

    var displaySearchContinueBtn = function displaySearchContinueBtn() {
        window.setTimeout(function() {
            var $btn = $('#search-continue');
            $btn.fadeIn(1000);
    ...
    var stepsDuration = { verySlow: unit * 5, slow: unit * 4, middle: unit * 3, fast: unit * 2, veryFast: unit * 1 };
    var step = function(percentaje, duration, easing) {
        var displayPercentage = true;
        return function(next) {
            $bar.animate({ width: percentaje + '%' }, {
                duration: duration,
                easing: easing || 'linear',
                step: function(step) {
                    var percent = Math.floor(step);
                    var $progressPercentangeText = $('.progress-text-inner');
                    var progressPercentangeOffset = 10;
                    $progressPercentangeText.html(percent.toString() + "%");
                    ...
                    if (displayPercentage && $progressPercentangeText.width() + progressPercentangeOffset < $bar.width()) {
                        $progressPercentangeText.fadeIn();
    ...
    var progress = $bar
        .queue(STEPS, step(15, stepsDuration.veryFast, 'swing'))
    ...
    setInterval(function() { $('.time-remaining-count').html(timeRemaining); }, 15000);
    ...
    var findDynamicContent = function() { return $("[data-bv-content]"); };
    var displayTargetedContent = function(queryArgs, $dynamicElems) {
        ...
        _.forEach($dynamicElems, function(elem) {
            ...
            if (!$target || $target.length === 0) { $defaults.show(); } else {
                $defaults.hide();
                $target.show();
            }
        });
    ...
    function initializeTestimonials() {
        ...
        var testimonialsSwitcher = function() {
            var $item = $('.test-act:first');
            $item.remove();
            $('.test-act.hidden:first').fadeIn('fast');
            $('.test-act.hidden:first').removeClass('hidden');
            if (++testSwitchNum < numTestimonals) { setTimeout(testimonialsSwitcher, TESTIMONIAL_DURATION); }
        };
